...
func (pg *PostgresWorkoutStore) CreateWorkout(workout *Workout) (*Workout, error) {
	...
	for _, entry := range workout.Entries {
		query := `INSERT INTO workout_entries (workout_id, exercise_name, reps, sets, duration_seconds, weight, notes, order_index)
		Values ($1, $2, $3, $4, $5, $6, $7, $8)
		RETURNING id`
		err = tx.QueryRow(query, workout.ID, entry.ExerciseName, entry.Reps, entry.Sets, entry.DurationSeconds, entry.Weight, entry.Notes, entry.OrderIndex).Scan(&entry.ID)
		if err != nil {
			return nil, err
...
func (pg *PostgresWorkoutStore) GetWorkoutByID(id int64) (*Workout, error) {
	...
	//get entries
	entriesQuery := `
	SELECT id,  exercise_name, reps, sets, duration_seconds, weight, notes, order_index
...
func (pg *PostgresWorkoutStore) UpdateWorkout(workout *Workout) error {
	...
	for _, entry := range workout.Entries {
		...
		_, err = tx.Exec(query,
			...
			entry.Notes,
			entry.OrderIndex,
		) //we use $1, $2, etc. to prevent SQL injection and ensure proper handling of data types
	...
	return tx.Commit() //this will commit the transaction if all operations were successful, otherwise it will be rolled back due to the deferred tx.Rollback() call
}
